	...
	<div class="collapse navbar-collapse" id="navbarSupportedContent">
		...
		<form action="in" class="form-login" method="post">
			...
			<c:set var="emaill" scope="session"
				value="#session.reader.getUsername()" />
		...
		<form action="out" class="form-logout" method="post">
			<c:if test="${not empty emaill}">
				...
				<a class="btn btn-outline-primary my-2 my-sm-0" id="btn-login"
					href="profile.jsp"> <font color='white'> <s:property
							value="#session.reader.getUsername()"></s:property></font> <!-- 获取session中reader的username -->
				</a>
			</c:if>
		</form>
	</div>
